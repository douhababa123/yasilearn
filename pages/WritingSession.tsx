...
import { Lightbulb, Layout, Undo, Redo, Maximize, ArrowRight, Clock, Info, Quote, Loader2 } from 'lucide-react';
...
export const WritingSession = () => {
  ...
  const topic = "Some people believe that the best way to solve environmental problems is to increase the price of fuel. To what extent do you agree or disagree?";
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 调用 AI 服务层
      const result = await evaluateEssay(essayContent, topic);
    ...
    <div className="flex h-screen bg-[#f6f6f8] overflow-hidden flex-col">
       ...
       <header className="h-16 px-6 lg:px-8 border-b border-slate-200 bg-white flex items-center justify-between shrink-0 z-10">
        <div className="flex items-center gap-4">
          <div>
            <h1 className="text-lg font-bold text-slate-900 leading-tight">Writing Task 2: Essay</h1>
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <span>Topic: Environmental Issues</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex flex-col items-end mr-2">
            <span className="text-xs font-medium text-slate-400">Last saved just now</span>
          </div>
          <button 
